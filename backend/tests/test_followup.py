...
def test_get_due_follow_ups(client, admin_headers, sample_contact_for_regular, admin_user, db):
    ...
    response = client.get("/contacts/follow-ups/due?days_ahead=7", headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) >= 1
...
def test_get_due_follow_ups_custom_days(client, admin_headers, admin_user, db):
    contact = Contact(
        id=generate_id(),
        first_name="Far",
        last_name="Away",
        ...
        status="cold",
        needs_follow_up=True,
        follow_up_date=datetime.now(timezone.utc) + timedelta(days=20),
        assigned_user_id=admin_user.id,
        created_at=datetime.now(timezone.utc),
    )
    db.add(contact)
    ...
    response = client.get("/contacts/follow-ups/due?days_ahead=30", headers=admin_headers)
    assert any(c["first_name"] == "Far" for c in response.json())


def test_get_due_follow_ups_only_own(client, user_headers, admin_user, regular_user, db):
...
def test_get_overdue_follow_ups(client, admin_headers, admin_user, db):
    contact = Contact(
        id=generate_id(),
        first_name="Overdue",
        last_name="Contact",
...
def test_get_overdue_follow_ups_only_own(client, user_headers, admin_user, regular_user, db):
    admin_contact = Contact(
        id=generate_id(),
