...
def test_update_contract(client, admin_headers, sample_contract):
    response = client.put(f"/contracts/{sample_contract.id}", json={
...
def test_update_contract_not_found(client, admin_headers):
    response = client.put("/contracts/nonexistent-id", json={
        "title": "X",
        "source": "X",
        "deadline": "2026-06-01T00:00:00",
        "status": "prospective",
...
def test_import_samgov_basic(client, admin_headers):
    response = client.post("/contracts/import/samgov", json={
        "opportunities": [{
            "noticeId": "SAM-001",
...
def test_import_samgov_duplicate_skipped(client, admin_headers):
    # Import first time
    client.post("/contracts/import/samgov", json={
        "opportunities": [{
            "noticeId": "SAM-DUP",
